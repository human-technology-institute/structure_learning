...
smcmc_Res_folder = r"F:\Gilad\SUSAN - AISTATS 2025\structure_learning _results\UTS Laptop\JS\structure"
smcmc_folders = ["2024-10-08_18-09-58_ER1", "2024-10-08_19-00-32_ER2", "2024-10-08_19-53-26_ER3"]
...
for folder in smcmc_folders:
    ...
    for exp_i in range(n_exp):
        ...
        MCMC_JS_results_dict.update({f"JS_MCMC_{exp_i}": JS_MCMC_res})
        OM_JS_results_dict.update({f"JS_OM_{exp_i}": JS_OM_res})
        OM_JS_accepted_results_dict.update({f"JS_OM_accepted_{exp_i}": JS_OM_accepted_res})
    ...
    # Plotting
    df_MCMC = pd.read_csv(os.path.join(RESPATH, 'MCMC_JS_results.csv'), index_col=0)
    df_OM = pd.read_csv(os.path.join(RESPATH, 'OM_JS_results.csv'), index_col=0)
    quantiles = [0.05, 0.95]

    df_mcmc_JS_only = df_MCMC.drop(columns=["sample_num"])
    df_OM_JS_only = df_OM.drop(columns=["sample_num"])

    MCMC_mean = df_mcmc_JS_only.mean(axis=1)
    MCMC_std = df_mcmc_JS_only.std(axis=1)

    MCMC_quantiles = df_mcmc_JS_only.apply(lambda row: row.quantile(quantiles), axis=1)

    OM_mean = df_OM_JS_only.mean(axis=1)
    OM_std = df_OM_JS_only.std(axis=1)
    OM_quantiles = df_OM_JS_only.apply(lambda row: row.quantile(quantiles), axis=1)

    plt.plot(df_MCMC["sample_num"], MCMC_mean, color='blue', label='MCMC')
    plt.fill_between(df_MCMC["sample_num"], MCMC_quantiles[quantiles[0]], MCMC_quantiles[quantiles[1]], color='blue',
                     alpha=0.2)
    plt.plot(df_OM["sample_num"], OM_mean, color='red', label='OM')
    plt.fill_between(df_OM["sample_num"], OM_quantiles[quantiles[0]], OM_quantiles[quantiles[1]], color='red', alpha=0.2)
